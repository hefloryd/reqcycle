...
public abstract class AbstractPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	protected ITypesManager typesManager = ZigguratInject.make(ITypesManager.class);
	protected ITypesConfigurationProvider provider = ZigguratInject.make(ITypesConfigurationProvider.class);
	...
	protected void addDefaultButton(Composite composite_1) {
		...
		defaultButton.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
	...
	private String getText(EObject eobject, final ILabelProvider labelProvider, EStructuralFeature f) {
		...
		if (eGet instanceof Collection) {
			...
			return Joiner.on(", ").join(Iterables.transform(collec, new Function<Object, String>() {
