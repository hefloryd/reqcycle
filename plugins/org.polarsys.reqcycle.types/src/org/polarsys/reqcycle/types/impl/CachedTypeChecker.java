...
/**
 * This class keeps a cache of type of elements the cache is reinitialized each time a resource is modified
 * 
 * @author tfaure
 * 
 */
...
public class CachedTypeChecker implements ICachedTypedChecker, IReachableListener {
	...
	Cache<Reachable, ReachableStruct> cache = CacheBuilder.newBuilder().expireAfterAccess(30000, TimeUnit.MILLISECONDS).removalListener(new RemovalListener<Reachable, ReachableStruct>() {

		@Override
		public void onRemoval(RemovalNotification<Reachable, ReachableStruct> arg0) {
			arg0.getValue().clear();
			listener.removeReachableListener(CachedTypeChecker.this, arg0.getKey());
		}
	}).build();
		...
		try {
			res = cache.get(trimFragment, new Callable<ReachableStruct>() {
				...
				public ReachableStruct call() throws Exception {
					...
					listener.addReachableListener(trimFragment, CachedTypeChecker.this);
					return res;
				}
			});
			if (res == null) {
